...
function addBlockLink(user) {
  'use strict';
  var blockedUsers = getBlockedUsers(), blockedUserIDs = [], blockText = "Block this user";
...
function blockUsersInThread(usersToBlock) {
  ...
  if (blockedUsers) {
    var userIDsToBlock = [], blockedUserIDs = [];
...
function blockUsersInThreadsList() {
  ...
  if (blockedUsers) {
    ...
    blockedUsers.forEach(function (value, index, array) {
      var threadsToBlock = $('.thread_by_' + value.userid);
    ...
    if ($('.blocked_thread').size() > 0) { // TODO: No wonder this is slow.
      var blockedThreadsBody = $(document.createElement('tbody'));
      blockedThreadsBody.attr("id", "blocked_threads").appendTo('#tww_thread_table').hide();

      var threadTableFoot = $(document.createElement('tfoot'));
      threadTableFoot.attr("id", "tww_thread_table_footer");
      var threadTableFootRow = $(document.createElement('tr')).appendTo(threadTableFoot).css("background-color", "#FFAAAA"); // TODO: Use user preference for highlight color?
      var threadTableFootCell = $(document.createElement('td')).appendTo(threadTableFootRow).attr("colspan", "6").attr("align", "center");
      var blockedThreadsLink = $(document.createElement('a')).attr("id", "show_blocked_threads_link").attr("href", "#").addClass("plain");
...
function getBlockedUsers() {
  'use strict';
  var blockedUsers = GM_getValue("blocked_users", JSON.stringify(null));
...
function renderBlockedUsers(target) {
  ...
  if (blockedUsers) {
    blockedUsers.forEach(function (value, index, array) {
      var blockedUser = new User(value.username, value.userid);
...
function toggleUserOnBlockList(user) {
  'use strict';
  var blockedUsers = getBlockedUsers(), blockedUserIDs = [], result;
  ...
  if (blockedUsers) {
    ...
    var userInBlockedList = jQuery.inArray(user.userid, blockedUserIDs);
